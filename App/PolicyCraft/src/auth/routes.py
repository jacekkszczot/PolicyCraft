...
import logging
print("=== AUTH ROUTES MODULE LOADED ===")
...
def logout():
    ...
    print(f"DEBUG: Before logout - current_user.is_authenticated: {current_user.is_authenticated}")
    print(f"DEBUG: Before logout - current_user.username: {current_user.username}")
    print(f"DEBUG: Session before: {dict(session)}")
    print("DEBUG: LOGOUT_USER() CALLED!")
    
    username = current_user.username if current_user.is_authenticated else 'Unknown'
    
    # Try multiple logout methods
    logout_user()
    print(f"DEBUG: After logout_user() - current_user.is_authenticated: {current_user.is_authenticated}")
    
    # Force clear Flask-Login session
    if '_user_id' in session:
        session.pop('_user_id', None)
        print("DEBUG: Removed _user_id from session")
    
    if '_fresh' in session:
        session.pop('_fresh', None)
        print("DEBUG: Removed _fresh from session")
    
    # Clear all session data
    session.clear()
    print(f"DEBUG: After session.clear() - Session: {dict(session)}")
    print(f"DEBUG: After all - current_user.is_authenticated: {current_user.is_authenticated}")
