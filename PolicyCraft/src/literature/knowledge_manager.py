    ...
    def _parse_metadata_from_markdown(self, content: str) -> Dict:
        ...
        try:
            ...
            for line in lines:
                ...
                # Extract quality score (multiple formats including Relevance Score)
                elif any(pattern in line.lower() for pattern in ["**quality score**:", "quality score:", "quality:", "score:", "relevance score:"]):
                    try:
                        score_str = None
                        ...
                        if "**quality score**:" in line.lower():
                            ...
                            # Handle "*Relevance Score: X.XX*" format
                            if "*" in line:
                                # Extract content between asterisks
                                import re
                                match = re.search(r'\*Relevance Score:\s*([\d.]+)\*', line)
                                if match:
                                    score_str = match.group(1)
                                else:
                                    # Fallback: split by "relevance score:" and clean
                                    parts = line.split("relevance score:", 1)
                                    if len(parts) > 1:
                                        score_str = parts[1].strip().strip('*').strip()
                            else:
                                score_str = line.split("relevance score:", 1)[1].strip()
                        elif "score:" in line.lower():
                            # Only handle "score:" if it's not part of "relevance score"
                            if "relevance score:" not in line.lower():
                                score_str = line.split("score:", 1)[1].strip()
                        
                        # Convert to float if possible
                        if score_str:
                            try:
                                # Clean the score string (remove any remaining asterisks, spaces, etc.)
                                clean_score = score_str.strip().strip('*').strip()
                                metadata["quality_score"] = float(clean_score)
                            except ValueError:
                                logger.warning(f"Could not convert quality score to float: {score_str}")
                        else:
                            logger.debug(f"No score value extracted from line: {line}")
